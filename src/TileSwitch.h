...
class TileSwitch : public Tile {
    public:
    TileSwitch(Board* boardPtr, const Vector2u& position, bool noAdjacentUpdates = false, char charID = '\0', State state = LOW);    // Construct a switch tile, noAdjacentUpdates stops updates to adjacent tiles.
    ~TileSwitch();
